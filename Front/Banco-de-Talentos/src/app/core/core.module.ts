...
export class CoreModule implements OnInit {
  ...
  constructor(private masterService: MasterService) {
    this.masterService.getLanguages().subscribe(languages => {
      sessionStorage.setItem(MasterRespConst.STORAGE_CURRENT_LANG, JSON.stringify(languages));
    });

    this.masterService.getRoles().subscribe(roles => {
      sessionStorage.setItem(MasterRespConst.STORAGE_CURRENT_ROLES, JSON.stringify(roles));
    });

    this.masterService.getCurrencies().subscribe(coins => {
      sessionStorage.setItem(MasterRespConst.STORAGE_CURRENT_CURRENCIES, JSON.stringify(coins));
    });

    this.masterService.getProfiles().subscribe(profiles => {
      sessionStorage.setItem(MasterRespConst.STORAGE_CURRENT_PROFILES, JSON.stringify(profiles));
    });

    this.masterService.getLangProficiency().subscribe(levels => {
      sessionStorage.setItem(MasterRespConst.STORAGE_CURRENT_PROFICIENCY, JSON.stringify(levels));
    });
    ...
    this.masterService.getCountries().subscribe(countries => {
      ...
      sessionStorage.setItem(MasterRespConst.STORAGE_CURRENT_COUNTRY_CITY, JSON.stringify(countryCityList));
    });
